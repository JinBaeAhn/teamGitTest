...
public class HistoryService {
	...
	public HistoryPageData boardMyHistory(int memberNo, int reqPage) {
		Connection conn = JDBCTemplate.getConnection();
		int numPerPage = 10;
		int end = numPerPage*reqPage;
		int start = end - numPerPage + 1;
		//rnum포함 쿼리문 작성
		ArrayList<Board> boardList = dao.boardMyHistory(conn, memberNo, start, end);
		//페이징제작
		//1. 전체페이지수계산
		int totalCount = dao.boardHistoryCount(conn, memberNo);
		int totalPage = 0;
		if(totalCount%numPerPage == 0) {
			totalPage = totalCount/numPerPage;
		}else {
